...
export class Menu {
  constructor () {
    ...
    this.puzzle = new Puzzle(
      ...
      /* winHandler */ index => {
        ...
        window.localStorage.setObject('progression', menu.progression)
        try {
    ...
    this.loadLevels().then(() => {
      ...
      menu.progression = window.localStorage.getObject('progression')
  ...
  loadLevels () {
    ...
    var checkNextLevel = (index, listResolve, listReject) => {
      window.fetch('./json/level/' + index + '.json')
        .then(function (r) {
        ...
        .catch(error => {
          if (['403', '404', 'Failed to fetch'].includes(error.message)) {
            listResolve()
          } else {
            window.alert("Couldn't load puzzle " + index + '!')
            listReject()
          }
        })
    }
  ...
  loadLevel (index) {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        window.alert('Level ' + index + ' is not correctly defined!')
        console.error(error)
        reject(error)
      }
    })
